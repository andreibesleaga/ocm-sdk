...
import { APIPromise } from 'ocm-sdk/core/api-promise';

import util from 'node:util';
import Ocm from 'ocm-sdk';
import { APIUserAbortError } from 'ocm-sdk';
...
describe('logging', () => {
  ...
  const forceAPIResponseForClient = async (client: Ocm) => {
  ...
  test('warn when env var level is invalid', async () => {
    ...
    const client = new Ocm({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'OCM_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['OCM_BASE_URL'] = undefined;
    });
