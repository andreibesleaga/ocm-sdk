...
import { Comment, CommentSubmitParams, CommentSubmitResponse } from './resources/comment';
import { Mediaitem, MediaitemCreateParams, MediaitemCreateResponse } from './resources/mediaitem';
...
export class Ocm {
  ...
  constructor({
    ...
    this.timeout = options.timeout ?? Ocm.DEFAULT_TIMEOUT /* 1 minute */;
  ...
  protected validateHeaders({ values, nulls }: NullableHeaders) {
    ...
    if (this.username && values.get('authorization')) {
      return;
    }
    ...
    throw new Error(
      'Could not resolve authentication method. Expected either apiKey or username to be set. Or for one of the "X-API-Key" or "Authorization" headers to be explicitly omitted',
    );
  }
  ...
  referencedata: API.Referencedata = new API.Referencedata(this);
  profile: API.Profile = new API.Profile(this);
  comment: API.Comment = new API.Comment(this);
